    ...
    async def analyze(self, user_profile: Dict) -> Dict:
        ...
        for supply in supplies:
            ...
            for supplier in india_results["suppliers"][:5]:  # Top 5
                ...
                if verification["confidence"] > 0.6:  # Include if confidence > 60%
                ...
                elif verification["confidence"] > 0.3:  # Include if confidence > 30% as unverified
                    supplier["verification"]["status"] = "unverified"
                    verified_suppliers.append(supplier)
                    self.log_execution("thinking", {"message": f"⚠️ {supplier.get('name', 'Supplier')} added unverified (confidence: {verification['confidence']:.0%})"})
                else:
                    self.log_execution("thinking", {"message": f"❌ {supplier.get('name', 'Supplier')} failed verification (confidence: {verification['confidence']:.0%})"})
...
Return ONLY valid JSON array."""
            ...
            result = await self.ollama.reasoning_task(fallback_prompt)
